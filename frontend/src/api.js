...
export async function fetchMaterialPipes(material) {
  const response = await fetch(`/api/materials/preview?material=${encodeURIComponent(material)}`);
  if (!response.ok) {
...
export async function fetchPipeGroupIds(pipe) {
  const response = await fetch(`/api/pipes/preview?pipe=${encodeURIComponent(pipe)}`);
  if (!response.ok) {
...
export async function fetchSpec(pipe, id) {
  const response = await fetch(`/api/spec?pipe=${encodeURIComponent(pipe)}&id=${encodeURIComponent(id)}`);
  if (!response.ok) {
